export const statusValidator = (status) => {
    if (typeof status !== "string" || !["Not Done", "Done"].includes(status)) return { field: "status", message: "status must be either of Not Done or Done" }
    
    return null;
}
export const priorityValidator = (priority) => {
    if (typeof priority !== "string" || !["Low", "Medium", "High"].includes(priority)) return { field: "priority", message: "priority must be either of Low, Medium or High" }
    
    return null;
}
export const descriptionValidator = (description) => {
    if (description.length < 3) return { field: "description", message: "Description must be of atleast 3 character" }
...
export const indexValidator = ({ index }) => {
    ...
    if (!index) {
        ...
        errors.push({ field: "index", message: "Todo Index must be a valid number." })
